...
### ⚠️ 최우선 원칙: 구현 후 반드시 테스트 (MUST)

> 이 원칙은 다른 모든 지침에 우선하며, 생성되는 모든 ROADMAP.md와 작업 정의에 예외 없이 반영되어야 합니다.

1. **구현 후 테스트는 선택이 아닌 필수**: 모든 구현 작업(Task)은 구현을 완료한 직후 반드시 테스트를 수행해야 하며, 테스트 없이 완료(✅) 처리할 수 없습니다.
2. **테스트 도구는 Playwright MCP**: 모든 기능 검증 및 E2E 테스트는 **Playwright MCP**를 사용합니다. (브라우저 탐색, 요소 상호작용, 네트워크 요청 확인, 스크린샷 검증 등)
3. **API 연동·비즈니스 로직은 꼼꼼한 테스트 필수**: API 연동, 데이터 파싱/변환, 계산 로직, 상태 처리 등 비즈니스 로직을 포함하는 작업은 정상 케이스뿐 아니라 **실패 케이스·엣지 케이스·경계값**까지 빠짐없이 검증하는 상세한 테스트 시나리오를 작성해야 합니다.
4. **테스트 통과가 완료 조건**: 테스트를 통과해야만 해당 단계를 완료로 표시하고 다음 단계로 진행할 수 있습니다.
...
- 수락 기준과 완료 조건 정의
- **구현 완료 직후 반드시 테스트를 수행** (테스트 없는 완료 처리 금지)
- **모든 테스트는 Playwright MCP로 수행** (E2E 검증, 네트워크 요청 확인, UI 상호작용 검증)
- **API 연동·비즈니스 로직 구현 시 정상/실패/엣지 케이스를 모두 포함한 꼼꼼한 테스트 필수**
- 테스트 통과를 확인한 후에만 다음 단계로 진행하고, 결과를 작업 파일에 기록
...
- 작업 파일의 명세서를 따름
- 기능과 기능성 구현
- **구현을 완료한 후에는 반드시 테스트를 수행 (예외 없음)**
- **모든 테스트는 Playwright MCP를 사용하여 수행**
- **API 연동·비즈니스 로직은 정상 케이스 + 실패 케이스 + 엣지 케이스를 꼼꼼히 검증**
- 각 단계 후 작업 파일 내 단계 진행 상황 및 테스트 결과 업데이트
- 구현 완료 후 Playwright MCP를 사용한 E2E 테스트 실행
- 테스트 통과 확인 후 다음 단계로 진행 (테스트 미통과 시 완료 불가)
...
- 측정 가능한 완료 기준 제시
- **구현을 포함하는 모든 Task의 마지막 구현 사항은 Playwright MCP 테스트 항목으로 마무리** (예: "Playwright MCP로 ~ E2E 테스트")

### 🧪 테스트 작성 전략 (Playwright MCP 필수)

모든 ROADMAP과 작업 정의는 아래 테스트 전략을 반드시 반영해야 합니다.

#### **테스트 수행 원칙**

1. **구현 후 즉시 테스트**: 구현이 끝난 Task는 Playwright MCP 테스트를 수행해야 완료로 인정됩니다. "구현만 하고 테스트는 나중에"는 금지입니다.
2. **Playwright MCP 단일 도구 원칙**: 기능 검증·E2E 테스트는 Playwright MCP로 통일합니다. (페이지 탐색 → 요소 상호작용 → 결과/네트워크 검증 → 스크린샷)
3. **테스트 시나리오 명문화**: API/비즈니스 로직 Task의 작업 파일에는 `## 테스트 체크리스트` 섹션을 반드시 포함하고, 시나리오를 표/체크박스로 구체화합니다.

#### **API 연동·비즈니스 로직 테스트 깊이 (꼼꼼함 기준)**

API 연동, 데이터 파싱/변환, 계산 로직, 상태 처리를 포함하는 작업은 다음을 **모두** 검증합니다.

- **정상 케이스(Happy Path)**: 유효한 입력으로 기대 결과가 정확히 렌더링/반환되는지
- **실패 케이스**: API 오류(4xx/5xx), 타임아웃, 잘못된 응답 형식, 데이터 없음 상태 처리
- **엣지 케이스·경계값**: 빈 목록, 필수 속성 누락, 0/음수/최대값, 특수문자, 만료 데이터 등
- **데이터 정합성**: 계산 결과(합계/세금/금액 등)의 수치 정확성 검증
- **사용자 플로우**: 핵심 사용자 여정 전체를 처음부터 끝까지 E2E로 검증

#### **테스트 체크리스트 섹션 예시 (작업 파일용)**
...
- [ ] 유효한 ID 접속 시 데이터가 정상 렌더링되는지 확인
- [ ] 계산 결과(소계/세금/총액)가 정확한지 검증

### 실패·엣지 케이스

- [ ] 존재하지 않는 ID 접속 시 오류 페이지로 이동하는지 확인
- [ ] API 오류 응답 시 사용자 안내 메시지 표시 확인
- [ ] 필수 속성 누락/빈 목록 등 엣지 케이스 처리 확인

### 사용자 플로우 (E2E)

- [ ] 전체 사용자 여정을 Playwright MCP로 끝까지 검증
```
...
- [ ] 구현을 포함하는 모든 Task에 구현 후 Playwright MCP 테스트 단계가 명시되었는가?
- [ ] API 연동 및 비즈니스 로직 구현 Task에 정상/실패/엣지 케이스를 모두 포함한 꼼꼼한 테스트가 정의되었는가?
- [ ] 각 작업 파일에 "## 테스트 체크리스트" 섹션이 명시되었는가?
- [ ] 모든 테스트가 Playwright MCP를 사용하도록 작성되었는가?
- [ ] 모든 사용자 플로우에 대한 E2E 테스트 시나리오가 정의되었는가?
- [ ] 에러 핸들링, 엣지 케이스, 경계값, 데이터 정합성 테스트가 고려되었는가?
- [ ] 테스트 통과가 Task 완료(✅)의 전제 조건으로 명시되었는가?
